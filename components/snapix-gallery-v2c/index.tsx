...
export function SnapixGalleryV2() {
	...
	const loadImages = async (galleryId: string | null, force = false) => {
		...
		try {
			const gallery = galleryId
				? await client.getGallery({galleryId})
				: await client.getImagesWithoutGallery();
			const images = (gallery.gallery.images ?? []).map((entry) => entry.image);
	...
	// Load galleries and ungrouped images on mount
	React.useEffect(() => {
		void client.listGalleries()
			.then(res => {
		...
		<div className="flex min-h-svh flex-col gap-6 p-6 pb-16">
			<div className="flex flex-col gap-1.5">
				<h1 className="font-heading text-2xl font-medium flex gap-1">
					...
					<span>
						Snapix Gallery V2C (Image in Multiple Galleries Client-Side)
					</span>
				</h1>
